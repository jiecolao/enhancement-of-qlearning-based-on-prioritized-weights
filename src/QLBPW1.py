...
class QLBPW():
    ...
    def simulate_qlbpw(self):
        ...
        for e in range(self.episodes):
            # if e % 100 == 0 and self.dynamic_obs_enabled: self.generate_dynamic_obstacles()                                         # NOTE: For totally dynamic environment
            # if e == 699 and self.dynamic_obs_enabled: self.obstacles.append((1, 4))     # NOTE: For simulated dynamic environment
            ...
            if (e + 1) % ep_tracker == 0:
                ...
                print(f"Episode {e:3d} | Total Steps: {steps_taken+1:3d} | Total Reward: {episode_reward:6.2f} | Epsilon: {self.epsilon:.3f} | Elapsed: {elapsed:.2f}")
...
if __name__ == "__main__":
    ...
    print(f"\n=== QLBPW (State-space) Simulation ===\n\n")
    tracemalloc.start()
    start_time = time.time() 
    total_rewards = a.simulate_qlbpw()
    end_time = time.time() 
    elapsed_time = end_time - start_time

    # print_memory_stats("QLBPW final memory")
    print(f"\nQLBPW Total Runtime: {elapsed_time:.2f} seconds")
    print(f"QLBPW Total Rewards: {total_rewards}")
    tracemalloc.stop()
